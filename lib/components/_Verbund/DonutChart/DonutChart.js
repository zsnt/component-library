...
const renderCustomizedLabel = (both = true) => ({ cx, cy, midAngle, innerRadius, outerRadius, value, index }) => {
  ...
  const doubleSin = 1 - Math.sin(RADIAN * (450 - midAngle));
  ...
  const mx = index === 0 ? (cx - outerRadius * 1.15) : (cx + (innerRadius * 0.8 - doubleSin * innerRadius * 0.5) * cos);
  const my = index === 0 ? (cy - outerRadius * 1.1) : cy + (innerRadius * 0.8 - doubleSin * innerRadius * 0.5) * sin;
  const textAnchor = cos >= 0 ? 'start' : 'end';
  const textX = index === 0 ? (mx + fontSize * 2.3) : (cx - fontSize * 1.7);
  const textY = index === 0 ? (my - fontSize * 0.5) : (cy + fontSize / 4);
